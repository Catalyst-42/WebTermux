...
Теперь у нас есть F-droid, который нам больше не понадобиться и Termux. Дальнейшие действия мы будем провидить в последнем.

# Этап 2 - Обновление Termux
На этом этапе мы установим необходимые пакеты в Termux, для начала желательно обновить все уже установленные пакеты Termux:
...
Во время процесса установки вас могут несколько раз спросить разрешение конфликта версий. В принципе любой ваш ввод ничего не изменит, так что мы на все подобные вопросы будем отвечать пустым вводом `enter`, оставляя текущие версии файлов.
...
Теперь установим `openssh` и откроем порт 9000 для подключения ssg
...
Хорошо, ssh установлен, теперь мы можем попробовать подключиться к нашему телефону через компьютер. Только у нас пока что нету имени пользователя и ip адреса телефона, поэтоу для получения этих данных вы будем используем `whoami` и `ifconfig`
...
Теперь с компьютера под Linux или MacOS для подключения к нашему телефону мы вводим следующую команду:

```bash
ssh u0_a251@192.168.1.39 -p 9000
```

Если же вы используете Windows, то для подключения вы можете использовать программу [PuTTY](https://www.putty.org). Просто в полях для адреса и порта пропишите соответствующие значения.

![PuTTY](images/PuTTY.png)

В обоих случаях вас попросят сначала попросят внести новый адрес в список знакомых хостов, вводим yes или нажимаем agree. После чего нас попросят ввести пароль пользоватяля. Вводим тот пароль, который мы устанавливали через `passwd`.

Если всё было введно верно, то теперь вы увидите знакомый терминал нашего Termux'а. 

# Этап 3 - Настройка конфигов
...
После установки пакетов давайте создадим папку нашего сайта. Эта папка будет является корневой. При желании вы можете расположить эту папку в любом угодном вам месте, я же оставлю её в корневой папке:
...
#
Далее нам надо будет отредактировать три конфигурационных файла. Вы можете пойти двумя путями. Либо просто взять и через vim полностью их перезаписать, либо отредактировать все элементы каждого конфиг файла вручную, чтобы понимать, что вы меняете, как и на что это влияет. Если вы выбираете быстрый и лёгкий путь, то просто замените содержимое файлов по их адресам на те файлы, которые оставлены в этой папке под тем же именем.

Для полной перезаписи файла сначала скопируйте нужное содержимое, потом откройте каждый файл по очереди в vim и выполните следующее сочетание клавишь: `V`, `G`, `d`, `i`, вставьте нужный текст, затем `esc`, `:wq`. Вся эта последовательность должна будет выделить (V) первую строчку, потом весь текст (G), удалить его (d), после чего в режиме ввода (i) вы вставите новый текст, затем выйдите из режима ввода (esc), сохраните (w) и закроете (q) файл. Так для каждого из следующих файлов:
...
После этого переходите к 4 этапу.
...
Перейдём к нижнему vhost'у. Это будет наш отдельный хост для подключения к консоли phpmyadmin. У него мы меняем сам порт прослушки с `8080` на `3360`. Теперь поле `DocumentRoot` заменяем на адрес phpmyadmin: `/data/data/com.termux/files/usr/share/phpmyadmin` и также меняем `ServerName` на localhost. 
...
Теперь при заходе на адрес нашего телефона через порт 8080 мы будем попадать в папку сайта `Web`. А если укажем порт 3360, то попадём в PMA. Этим мы займёмся совсем скоро.

С файлом vhost'ов мы закончили, теперь выйдем в режим команд через `esc`, затем сохраним файл и выйдем из него: `:wq`.
...
# Этап 4 - Тестирование Apache и Mariadb
...
# Ответвление 3 - Сломанная база данных
...
Для того, чтобы вернуть всё обратно в рабочее состояние придётся удалить папку со сломанной базой данных и создать её заново в PhpMyAdmin.

Так что обязательно перезагружайте телефон только после того, как будет безопасно закрыт sql сервер через 
