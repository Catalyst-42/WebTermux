Несколько лет назад я увидел в интернете видео, как из raspberry pi сделать домашний web сервер. Тогда мне эта идея очень понравилась. И вот я уже собрался сам выполнить этот гайд... как понял, что у меня нет самого главного - это чёртовой коробки. Тогда я ещё долго думал, всё же стоит её покупать или нет, в итоге от покупки я отказался. 

И правильно сделал, потому что создать свой домашний сервер можно и на телефоне без особых проблем. Итак, в этом видео я расскажу как создать локальный web сервер с базой данных sql на вашем телефоне под управлением Android.

Всё это дело мы будем проворачивать с самого нуля, по ходу дела я буду объяснять что и зачем мы делаем.

План руководства будет следующий: сначала установим необходимые приложения на телефон. Потом обновим и настроим Termux, после чего я сделаю одну опциональную вещь - запущу ssh на телефоне и дальнейшие действия буду совершать через компьютер. Затем мы вернёмся к основному маршруту, установим apache, PhpMyAdmin и базу данных mariadb. Затем настроим конфиг файлы и протестируем готовую сборку. Весь билд в итоге у меня занял примерно полтора гигабайта памяти, так что заранее убедитесь, что у вас хватит места всё это поставить. 
